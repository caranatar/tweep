    ...
    pub fn parse(context: FullContext) -> Output<Result<Self, ErrorList>> {
        ...
        for (c, e, w) in vec![
            ...
            if indices.is_err() {
                ...
                for idx in indices {
                    let warning = Warning::new(w.clone(), context.subcontext(ContextPosition::new(1, idx + 1)..=ContextPosition::new(1, idx+2))).with_column(idx + 1);
                    #[cfg(not(feature = "issue-context"))]
                    {
...
fn parse_metadata(context: FullContext) -> Result<serde_json::Map<String, serde_json::Value>, Warning> {
    ...
    if res.is_ok() {
        ...
        let warning = Warning::new(WarningType::JsonError(err_string), context.subcontext(ContextPosition::new(1, col)..)).with_column(col);
        #[cfg(not(feature = "issue-context"))]
        {
            Err(warning)
        }
