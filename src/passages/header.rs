    ...
    pub fn parse(context: FullContext) -> Output<Result<Self, ErrorList>> {
        ...
        if let Some(range) = guess_metadata_range(input) {
            ...
            if find_last_unescaped(&input[range.end..], "[").is_some() {
                let error = Error::new(ErrorType::MetadataBeforeTags, context.subcontext(Position::new(1, pos+1)..));
                errors.push(error);
            }

            let meta_context = context.subcontext(Position::new(1, range.start)..=Position::new(1, range.end));
        ...
        if let Some(pos) = find_last_unescaped(&input[..name_end_pos], "[") {
            ...
            if let Some(p) = end_pos {
                ...
                let error = Error::new(ErrorType::UnclosedTagBlock, context.subcontext(Position::new(1, pos+1)..));
                errors.push(error);
            }
        ...
        for (c, e, w) in vec![
            ...
            let indices = check_name(context.subcontext(Position::new(1,1)..=Position::new(1, name_end_pos)), c, e);
            if indices.is_err() {
                ...
                for idx in indices {
                    let warning = Warning::new(w.clone(), context.subcontext(Position::new(1, idx + 1)..=Position::new(1, idx+2)));
        ...
        if name.is_empty() {
            let error = Error::new(ErrorType::EmptyName, context.subcontext(Position::new(1,3)..));
            errors.push(error);
        }
...
fn parse_metadata(context: FullContext) -> Result<serde_json::Map<String, serde_json::Value>, Warning> {
    ...
    if res.is_ok() {
        ...
        let warning = Warning::new(WarningType::JsonError(err_string), context.subcontext(Position::new(1, col)..));
        Err(warning)
    }
}
...
fn check_name(context: FullContext, unescaped_str: &str, error: ErrorType) -> Result<Vec<usize>, Error> {
    ...
    if unescaped.is_empty() {
        ...
        let err_range = Position::new(1, unescaped[0] + 1)..=Position::new(1, unescaped[0]+1);
...
mod tests {
    ...
    fn empty_name() {
        ...
        let expected = context.subcontext(Position::new(1, 3)..);
        let out = PassageHeader::parse(context);
        let (res, _) = out.take();
        assert_eq!(res.is_err(), true);
        assert_eq!(res.err().unwrap().errors[0], {
    ...
    fn metadata_before_tags() {
        ...
        let expected = context.subcontext(Position::new(1, 22)..);
        let out = PassageHeader::parse(context);
        let (res, _) = out.take();
        assert_eq!(res.is_err(), true);
        assert_eq!(res.err().unwrap().errors[0], {
    ...
    fn unescaped_chars() {
        for (c, e) in vec![
            ...
            assert!(errors.iter().any(|a| {
                let sub = sub.subcontext(Position::new(1, 4)..=Position::new(1, 4));
            ...
            assert_eq!(res.is_err(), true);            
            assert!(res.err().unwrap().errors.iter().any(|a| {
                let sub = sub.subcontext(Position::new(1,6)..=Position::new(1,6));
                let expected = Error::new(e.clone(), sub);
                *a == expected
            }));
        }
