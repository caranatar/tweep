    ...
    pub fn parse(context: FullContext) -> Output<Result<Self, ErrorList>> {
        ...
        if let Some(range) = guess_metadata_range(input) {
            ...
            if find_last_unescaped(&input[range.end..], "[").is_some() {
                let error = Error::new(ErrorType::MetadataBeforeTags, context.subcontext(ContextPosition::new(1, pos+1)..));
                errors.push(error);
            }

            let meta_context = context.subcontext(ContextPosition::new(1, range.start)..=ContextPosition::new(1, range.end));
        ...
        if let Some(pos) = find_last_unescaped(&input[..name_end_pos], "[") {
            ...
            if let Some(p) = end_pos {
                ...
                let error = Error::new(ErrorType::UnclosedTagBlock, context.subcontext(ContextPosition::new(1, pos+1)..));
                errors.push(error);
            }
        ...
        for (c, e, w) in vec![
            ...
            if indices.is_err() {
                ...
                for idx in indices {
                    let warning = Warning::new(w.clone(), context.subcontext(ContextPosition::new(1, idx + 1)..=ContextPosition::new(1, idx+2)));
        ...
        if name.is_empty() {
            let error = Error::new(ErrorType::EmptyName, context.subcontext(ContextPosition::new(1,3)..));
            errors.push(error);
        }
...
fn parse_metadata(context: FullContext) -> Result<serde_json::Map<String, serde_json::Value>, Warning> {
    ...
    if res.is_ok() {
        ...
        let warning = Warning::new(WarningType::JsonError(err_string), context.subcontext(ContextPosition::new(1, col)..));
        Err(warning)
    }
}
